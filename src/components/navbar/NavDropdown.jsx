...
export default function NavDropdown({ isMobile, onCloseMenu, titleEn, titleAr, items }) {
    const [isOpen, setIsOpen] = useState(false);
    const { isRtl } = useThemeContext();
    const dropdownRef = useRef(null);
    const location = useLocation();

    // Check if any link in this dropdown is currently active
                    ...
                    <div className={`flex flex-col mt-1 space-y-1 ${isRtl ? 'pr-4 border-r-2 border-primary/20 mr-2' : 'pl-4 border-l-2 border-primary/20 ml-2'}`}>
                        {items.map(item => (
                            <Link
                                ...
                                className={`px-4 py-2.5 rounded-lg text-sm transition-colors ${location.pathname === item.path
                ...
                <div className="py-2 flex flex-col">
                    {items.map(item => (
                        <Link
                            key={item.path}
                            to={item.path}
                            className={`px-4 py-2.5 text-sm transition-colors hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-primary ${location.pathname === item.path
                                    ? 'text-primary font-bold bg-primary/5'
